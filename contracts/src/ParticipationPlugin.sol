...
import { IParticipationPlugin } from "./IReputationPlugin.sol";
...
contract ReputationPlugin is IReputationPlugin, Plugin, ERC20 {
    ...
    function _updateBalances(address /* from */, address /* to */, address from, address to, uint256 amount) internal virtual {
        ...
        if (from != to && amount > 0) {
            if (from == address(0)) {
                ...
                uint minOf = amount < balanceOf(from) ? amount : balanceOf(from);
                _burn(from, minOf);
            } else {
